...
async function main() {
  ...
  // Copia src/i18n/*.json → dist/i18n/ (lazy-load lingue extra nel pluginLoader)
  const i18nSrc = join(abs, 'src/i18n')
  if (existsSync(i18nSrc)) {
    const i18nDst = join(outDir, 'i18n')
    mkdirSync(i18nDst, { recursive: true })
    const files = readdirSync(i18nSrc).filter(f => f.endsWith('.json'))
    for (const f of files) copyFileSync(join(i18nSrc, f), join(i18nDst, f))
    console.log(`  i18n: ${files.join(', ')} → dist/i18n/`)
  }

  console.log(`✓ dist/${fsKey}.js ready`)

  // Rigenera automaticamente il manifest radice così è sempre in sync
  const { execSync } = await import('child_process')
  try {
    execSync('node tools/generate-manifest.mjs', { cwd: ROOT, stdio: 'inherit' })
  } catch {
    console.warn('⚠ generate-manifest.mjs fallito — aggiorna manifest.json manualmente')
  }
}
