...
import { Download, Trash2, CheckCircle2, AlertTriangle, RefreshCw, Plus } from 'lucide-react'
...
export default function ApplianceSettings({ cardId }) {
  ...
  // Alias: se l'utente ha un package installato con nome diverso dal suffix
  // corrente ma mappato allo stesso preset (es. pianocottura.yaml vs piano)
  // lo consideriamo "già installato" e re-allineiamo il suffix al filename.
  const aliasedSuffix = (() => {
    if (!s) return null
    ...
    // Scansione inversa: se un installed alias punta allo stesso preset
  ...
  // Auto-allinea il suffix al filename reale del package quando rileva un alias.
  useEffect(() => {
    if (aliasedSuffix && aliasedSuffix !== s) {
      update({ suffix: aliasedSuffix })
    }
  }, [aliasedSuffix, s])
